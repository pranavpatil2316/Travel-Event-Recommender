...
async function getAllEventsForCountryOrCity(country, city, season, duration) {
    ...
    // Save events to database and get with ratings
    const eventsWithStats = await Promise.all(allEvents.map(async (event) => {
        try {
            // Save event to database if not exists
            const eventId = await saveEventToDatabase(event);
            
            // Get event with stats from database
            const eventWithStats = await eventDB.getEventWithStats(eventId);
...
async function saveEventToDatabase(event) {
    try {
        ...
        const existingEvents = await eventDB.getEvents(event.city, event.category);
...
async function getAllEventsForCity(city, season, duration) {
    ...
    try {
        const dbEvents = await eventDB.getEvents(city);
...
async function getAllEventsForCountry(country, season, duration) {
    ...
    // First, try to get all events from database for the country
    try {
        const allDbEvents = await eventDB.getEvents();
        ...
        console.error('Error getting country events from database:', error);
    }
...
async function loadUpcomingEvents() {
    try {
        ...
        const upcomingEvents = await eventDB.getUpcomingEvents(null, parseInt(upcomingDays));
...
async function showEventDetails(eventId) {
    try {
        const event = await eventDB.getEventWithStats(eventId);
        if (!event) {
...
async function submitRating(eventId) {
    try {
        ...
        // Add rating
        await eventDB.addRating(eventId, currentUser.id, selectedRating);
        
        // Add review if text provided
        if (reviewText.trim()) {
            await eventDB.addReview(eventId, currentUser.id, selectedRating, reviewText.trim(), userName);
        }
